...
class nextcloud {
    ...
    public static function submission(string $wwwroot, string $ncadmin = null, string $ncpass = null) {
        ...
        if ($index !== false) {
            ...
            cfg::set('assignsubmission_tipnc', 'host', 'https://nextcloud.' . $defaulthost);
        }
     ...
     * @throws dml_exception
     */
    public static function modtipnc(string $wwwroot) {
        $index = strpos($wwwroot, 'moodle');
        if ($index !== false) {
            $defaulthost = substr($wwwroot, $index + 7);
            cfg::set('tipnextcloud', 'host_nextcloud', 'https://nextcloud.' . $defaulthost);
        }
